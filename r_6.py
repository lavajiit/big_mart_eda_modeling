...
models = [Ridge(alpha=0.5), Lasso(alpha=0.5)]
for each_model in models:
    ...
    cv_score = cross_val_score(
        each_model, X, y, cv=5, scoring='neg_mean_squared_error')
    mean_CV = np.sqrt(np.abs(np.mean(cv_score)))

    scores = pd.DataFrame({'': [rmse_train, rmse_test, mean_CV]}, index=[
                          'RMSE_train', 'RMSE_test', 'RMSE_CV'])
